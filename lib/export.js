const Export = {
  ...
  describeStep(step) {
    ...
    switch (step.action) {
      ...
      case 'note':
        return `📝 ${step.content || ''}`;
      case 'video':
        return `🎥 ${step.content || 'Video recorded'}${step.videoDurationMs ? ` (${(step.videoDurationMs / 1000).toFixed(1)}s)` : ''}`;
      default:
  ...
  generateSummary(steps, session) {
    ...
    for (const s of steps) {
      ...
      } else if (s.action === 'video') {
        actions.push(`Video recorded${s.videoDurationMs ? ` (${(s.videoDurationMs / 1000).toFixed(1)}s)` : ''}`);
      }
    }
  ...
  async generateJSON(sessionId, filters) {
    ...
    // Apply range filter (event timestamp window) — set from sidepanel range selection
    if (f.rangeStartTs != null || f.rangeEndTs != null) {
      events = events.filter(e => {
        if (f.rangeStartTs != null && e.timestamp < f.rangeStartTs) return false;
        if (f.rangeEndTs != null && e.timestamp > f.rangeEndTs) return false;
        return true;
      });
      // Collect in-range video IDs from filtered events. Videos are matched by
      // their EVENT timestamp (not IndexedDB timestamp) because the latter is
      // saved async in onstop and can be a few ms earlier than the event.
      const inRangeVideoIds = new Set();
      for (const e of events) {
        if (e.type === 'event:video' && e.videoId) inRangeVideoIds.add(e.videoId);
      }
      // Also filter screenshots in range. Videos use event-ID match (robust);
      // images/screenshots use IndexedDB timestamp (event and IndexedDB are
      // close in time for those).
      const rangeFiltered = screenshots.filter(s => {
        if (s.mediaType === 'video') return inRangeVideoIds.has(s.id);
        if (f.rangeStartTs != null && s.timestamp < f.rangeStartTs) return false;
        if (f.rangeEndTs != null && s.timestamp > f.rangeEndTs) return false;
        return true;
      });
      screenshots.length = 0;
      screenshots.push(...rangeFiltered);
    }
    ...
    // Split media into screenshots and videos
    const imageMedia = screenshots.filter(s => !s.mediaType || s.mediaType !== 'video');
    const videoMedia = screenshots.filter(s => s.mediaType === 'video');

    // Build screenshot lookup
    const screenshotMap = {};
    const screenshotEntries = [];
    let ssIdx = 0;
    for (const s of imageMedia) {
      ssIdx++;
    ...
    for (const v of videoMedia) {
      ...
      videoEntries.push({
        ...
        filename: `video-${vidIdx}.webm`,
        _blob: v.videoBlob || null,
      });
    }
    ...
    let stepNum = 0;
    let lastUrl = session.url;
    // Map video id → index for cross-reference in steps
    const videoIdToIndex = {};
    for (const v of videoEntries) videoIdToIndex[v.id] = v.index;
    for (const e of events) {
      ...
      } else if (e.type === 'event:video' && f.steps) {
        // Video events get a step entry so they're not "missing" from the report.
        // Independent of f.screenshots — the video file inclusion is separate.
        stepNum++;
        const videoStep = {
          kind: 'step', step: stepNum,
          t: this.relativeTime(e.timestamp, session.startTime),
          action: 'video',
          content: e.content || 'Video recorded',
          videoIndex: videoIdToIndex[e.videoId] || null,
        };
        if (e.videoDurationMs) videoStep.videoDurationMs = e.videoDurationMs;
        timeline.push(videoStep);
      }
    }
    ...
    // Videos
    if (f.screenshots && videoEntries.length > 0) {
      report.videos = videoEntries.map(v => ({
        id: v.id,
        index: v.index,
        timestamp: v.timestamp,
        filename: v.filename,
      }));
      report._videoFiles = videoEntries
        .filter(v => v._blob)
        .map(v => ({ filename: v.filename, blob: v._blob }));
    }

    return { debugReport: report };
  },

  async generateMarkdown(sessionId, filters, options = {}) {
    ...
    if (f.rangeStartTs != null || f.rangeEndTs != null) {
      const fmt = (ts) => {
        if (ts == null) return r.endTime ? 'end' : 'now';
      ...
      lines.push(`**Range filter:** ${fmt(f.rangeStartTs)} → ${fmt(f.rangeEndTs)}`);
    }
    lines.push('');
    ...
    if (r.steps && r.steps.length > 0) {
      ...
      r.steps.forEach(s => {
        ...
        if (s.videoIndex) {
          line += ` — see [Video ${s.videoIndex}](./video-${s.videoIndex}.webm)`;
        }
        lines.push(line);
      });
      lines.push('');
    ...
    if (r.networkRequests && r.networkRequests.length > 0) {
      ...
      // "All" mode + ZIP export: errors stay inline, full data goes to separate file
      if (options.networkFile) {
        ...
        lines.push(`_All ${r.networkRequests.length} requests with full bodies are in [${options.networkFile}](./${options.networkFile})._`);
        lines.push('');
      } else {
        if (f.networkErrorsOnly) {
          lines.push('## Network Errors');
    ...
    // Videos — only if non-empty
    if (r.videos && r.videos.length > 0) {
      lines.push('## Videos');
      r.videos.forEach(v => {
        lines.push(`### Video ${v.index} (${v.timestamp})`);
        lines.push(`- File: [${v.filename}](./${v.filename})`);
        lines.push('');
      });
    }
  ...
  // Build a dedicated markdown file for all network requests (with bodies).
  // Used by exportZip when the user picks "All requests" mode — keeps the main
  // report scannable while still shipping the full request/response data.
  generateNetworkMarkdown(networkRequests, session) {
    const lines = [];
    lines.push('# Network Requests');
    lines.push('');
    lines.push(`**Session URL:** ${session.url}  `);
    lines.push(`**Request count:** ${networkRequests.length}`);
    lines.push('');
    lines.push('---');
    lines.push('');

    for (const r of networkRequests) {
      lines.push(`### ${r.method} ${r.url} → ${r.status} (${r.duration}ms)`);
      lines.push(`**Time:** \`${r.t}\``);
      if (r.requestBody) {
        lines.push('');
